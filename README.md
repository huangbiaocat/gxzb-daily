...
- **v0.3.3**：
  - **每日页面胶囊标签交互升级**：支持地市、信源、环节、大类及状态特征（重点/加班/滞后）胶囊按钮一键点击联动过滤与高亮反选，辅以轻量气泡反馈与状态同步。
  - **页面构建生成器扩展**：`build_daily_page.py` 增加 `--all` 参数，支持一键批量重构历史所有日期页面并自动刷新归档日历。
  - **测试与生产端分发同步**：新增多维胶囊过滤自动化测试，同步更新 Windows / i5 生产端分发包与在线热更新通道。
- **v0.3.2**：
